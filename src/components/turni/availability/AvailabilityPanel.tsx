...
function getExpectedRiposi(collab: Collaboratore): number {
  ...
  if (tipo === 'ore') {
    ...
    return Math.ceil(ore / 4); // 4h = 1 slot
  }
  return collab.riposi_settimanali_custom ?? 2;
}

// Status types for collaborators
type CollaboratoreStatus = 'complete' | 'partial' | 'missing' | 'ferie';

// Helper to get closed days from azienda orario_apertura
function getClosedDays(orarioApertura?: OrarioApertura): number[] {
  if (!orarioApertura) {
    // Default: nessun giorno chiuso
    return [];
  }

  if (orarioApertura.tipo === 'fisso') {
    // Orario fisso = nessun giorno chiuso
    return [];
  }

  // Orario variabile: trova i giorni chiusi
...
// Check if a date is a holiday (basic Italian holidays)
...
export function AvailabilityPanel({
  ...
  // Check if collaboratore is in ferie for the whole week
  ...
  // Get ferie days for a collaboratore
  ...
  // Get status for a collaboratore
  ...
  // Get riposi days for a collaboratore (1-7)
  ...
  // Handle quick assign riposo with type selection
  ...
  // Day selection popover for rest type
  const DaySelector = ({
    collabId,
    day,
    isClosed,
  ...
  const CollaboratorRow = ({ collab }: { collab: Collaboratore }) => {
    ...
    // Get tipo riposo icon for display
      ...
      >
        ...
        <div className="flex gap-1 flex-wrap">
          {[1, 2, 3, 4, 5, 6, 7].map((day) => {
            ...
            // Available day - show selector (sempre cliccabile se non è già riposo o ferie)
            return (
              <DaySelector
                key={day}
                collabId={collab.id}
        ...
        <CardHeader className="pb-3 flex-shrink-0">
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <CardTitle className="text-sm font-semibold">Disponibilità</CardTitle>
              {isExpanded ? (
